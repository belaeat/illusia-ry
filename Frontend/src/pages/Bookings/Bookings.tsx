...
export const Bookings = () => {
  ...
  const fetchMyBookings = async () => {
    try {
      ...
      const response = await fetch(
        "http://localhost:5001/api/booking-requests/my-requests",
        {
          headers: {
  ...
  const handleCancelBooking = async (bookingId: string) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5001/api/booking-requests/${bookingId}/cancel`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const renderBookingCard = (booking: BookingRequest) => (
    <div key={booking._id} className="bg-gray-100 rounded-xl p-6">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Booking Request</h2>
        </div>
        <div
    ...
    <div className="max-w-7xl mx-auto mt-10 p-6 bg-white shadow-lg rounded-2xl">
      ...
      {bookings.length === 0 ? (
        ...
        <div className="space-y-10">
          ...
          <div>
            <h2 className="text-2xl font-bold mb-6 text-[#3EC3BA]">
              Active Bookings
            </h2>
